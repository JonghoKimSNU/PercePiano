...
class Logger(SummaryWriter):
  def __init__(self, logdir):
    ...
    print("tensorboard initialized at", logdir)
  ...
  def log_validation(self, reduced_loss, loss_dict, model, iteration, is_best = False, is_test = False):
    if is_test:
       prefix = "test"
    else:
        prefix = "validation"
    if is_best:
        self.add_scalar(f"{prefix}_best/total_loss", reduced_loss, iteration)
        for key in loss_dict.keys():
            self.add_scalar("{}_best/{}".format(prefix, key), loss_dict[key], iteration)
    else:
        self.add_scalar(f"{prefix}/total_loss", reduced_loss, iteration)
        #loss_dict = get_mean_of_loss_dict(loss_dict)
        for key in loss_dict.keys():
            self.add_scalar("{}/{}".format(prefix,key), loss_dict[key], iteration)
