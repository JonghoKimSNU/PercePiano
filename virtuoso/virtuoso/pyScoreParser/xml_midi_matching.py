...
def match_xml_to_midi(xml_notes, midi_notes):
    ...
    for candidates in candidates_list:
        if len(candidates) ==1:
            ...
            match_list.append([])
    # figure out not matched notes 
    # unmatched_midi_notes = [i for i in range(len(midi_notes)) if i not in match_list]
    return match_list


def make_xml_midi_pair(xml_notes, midi_notes, match_list):
    pairs = [{'xml': xml_notes[i], 'midi': midi_notes[match_list[i]], 'midi_idx': match_list[i] \
              } if match_list[i] != [] else [] for i in range(len(match_list))  ]
...
def find_unmatched_midi_notes(midi_notes, match_list):
    unmatched_midi_notes = [midi_notes[i] for i in range(len(midi_notes)) if i not in match_list]
    return unmatched_midi_notes
...
def match_score_pair2perform(pairs, perform_midi, corresp_list):
    ...
    for pair in pairs:
        ...
        if index_in_corresp == -1:
            ...
            index_in_perform_midi = find_by_attr(perform_midi, float(corresp_pair['alignOntime']),  int(corresp_pair['alignPitch'])) # it doesn't find. return []
    ...
    assert len(match_list) == len(pairs)
    return match_list
...
def find_by_key(alist, key1, value1, key2, value2):
    for i, dic in enumerate(alist):
        if abs(float(dic[key1]) - value1) < 0.05 and int(dic[key2]) == value2: # looser value
            return i
    return -1
...
def find_by_attr(alist, value1, value2):
    for i, obj in enumerate(alist):
        if abs(obj.start - value1) < 0.05 and obj.pitch == value2: # looser value
            return i
    return []
