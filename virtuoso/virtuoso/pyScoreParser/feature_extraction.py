...
class PerformExtractor:
    ...
    # TODO: beat level tempo here
    def get_beat_tempo(self, piece_data, perform_data): # beat에 가까운 onset
        ...
        return [math.log(utils.get_item_by_xml_position(tempos, note).qpm, 10) for note in piece_data.xml_notes] #qpm??
    ...
    def get_qpm_primo(self, piece_data, perform_data, view_range=10):
        ...
        qpm_primo = 0
        view_range = min(view_range, len(perform_data.tempos))
    ...
    def get_articulation(self, piece_data, perform_data): # 원래 음에 비해서 얼마나 짧은가/긴가
        features = []
        if 'beat_tempo' not in perform_data.perform_features:
            perform_data.perform_features['beat_tempo'] = self.get_beat_tempo(piece_data, perform_data)
    ...
    def get_onset_deviation(self, piece_data, perform_data): #tempo beat 기준 Onset 시작 시간
    ...
    def get_start_time(self, piece_data, perform_data):
        '''
        :param piece_data:
        :param perform_data:
        :return: List of MIDI velocities of notes in score-performance pair.
        ...
        for pair in perform_data.pairs:
            if pair == []:
                ...
                start_time = pair['midi'].start
                #prev_start_time = start_time
            features.append(start_time)
        return features

    def get_end_time(self, piece_data, perform_data):
        '''
        :param piece_data:
        :param perform_data:
        :return: List of MIDI velocities of notes in score-performance pair.
        ...
        for pair in perform_data.pairs:
            if pair == []:
                ...
                end_time = pair['midi'].end
                #prev_end_time = end_time
            features.append(end_time)
        return features

    # def get_mmidi_pitch(self, piece_data, perform_data):
    #     '''
    #     :param piece_data:
    #     :param perform_data:
    #     :return: List of MIDI velocities of notes in score-performance pair.
    ...
    def get_mmidi_pitch(self, piece_data, perform_data):
        mmidi_pitch = []
        return [note.pitch for note in perform_data.midi_notes]
    
    def get_mmidi_velocity(self, piece_data, perform_data):
        return [note.velocity for note in perform_data.midi_notes]
    
    def get_mmidi_start_time(self, piece_data, perform_data):
        return [note.start for note in perform_data.midi_notes]
    
    def get_mmidi_end_time(self, piece_data, perform_data):
        return [note.end for note in perform_data.midi_notes]