...
DEFAULT_SCORE_FEATURES = ['midi_pitch', 
                          'duration', 'beat_importance', 'measure_length', 'qpm_primo',
...
DEFAULT_PURE_PERFORM_FEATURES = ['mmidi_velocity', 'mmidi_pitch', 'mmidi_start_time', 'mmidi_end_time']
# VNET_OUTPUT_KEYS = ('beat_tempo', 'velocity', 'onset_deviation', 'articulation', 'pedal_refresh_time', # articulation, beat_tempo에 start time, end time이 들어감
#                             'pedal_cut_time', 'pedal_at_start', 'pedal_at_end', 'soft_pedal',
#                             'pedal_refresh', 'pedal_cut') #added

# total data class
class DataSet:
    def __init__(self, path, save=False, features_only=False, align=False, split = ""):
        self.path = path
        self.split = split
        self.pieces = []
        self.scores = []
    ...
    # def extract_all_features(self, save=False): # save = True로 들어옴
    ...
    #         except Exception as ex:
    #             print(f'extract score features Error while processing {piece.meta.xml_path}. Error type :{ex}')
    ...
    #             except Exception as ex:
    #                 print(f'extract all features Error while processing {perform.midi_path}. Error type :{ex}')
...
# score data class
class PieceData:
    def __init__(self, xml_path, perform_lists, score_midi_path=None, composer=None, save=False, features_only=False, align=False, labels = None):
        if score_midi_path == None:
            raise Exception('score_midi_path is None')
            # score_midi_path = os.path.dirname(xml_path) + '/' + Path(xml_path).stem + '_score.mid'
        self.meta = PieceMeta(xml_path, perform_lists=perform_lists, score_midi_path=score_midi_path, composer=composer, labels = labels)
        self.performances = []
        self.labels = labels
        path = Path(xml_path)
        ...
        if features_only: # False
            ...
            for perform in perform_lists:
                ...
                if feature_path.exists():
                    ...
                    feature_path =  Path(perform).parent / Path(perform).name.replace('.midi', '_feature.dat')
        else:
            if save:
            ...
            for perform in perform_lists: # TODO: 이걸 list가 아니도록 수정?
    ...
    def _align_perform_with_score(self, perform):
        print('Performance path is ', perform.midi_path, "Score path is ", self.meta.xml_path)
        ...
        # assert len(perform.pairs) == len(perform.midi_notes), print("len(perform.pairs), len(perform.midi_notes)", len(perform.pairs), len(perform.midi_notes))
...
class PieceMeta:
    def __init__(self, xml_path, perform_lists, score_midi_path, composer=None, labels = None):
...
class ScoreData:
    def __init__(self, xml_path, score_midi_path, composer, read_xml_only=False):
        ...
        if "Beethoven_WoO80_thema_" in xml_path or \
            "Beethoven_WoO80_var3_" in xml_path or \
                "Beethoven_WoO80_var4_" in xml_path or \
                    "Beethoven_WoO80_var5_" in xml_path: 
            print()
    ...
    def _load_score_xml(self, xml_path):
        ...
        self.measure_positions = self.xml_obj.get_measure_positions()  # TODO: here call measure, beat position #FIXME: voice는 이거 전에 note별로 들어있음?
    ...
    def _match_score_xml_to_midi(self):
        ...
        self.unmatched_list = [i for i in range(len(self.score_midi_notes)) if i not in self.score_match_list]
...
class YamahaDataset(DataSet):
    ...
    def load_data_list(self):
        ...
        # composers = [xml.relative_to(self.path).parts[0] for xml in xml_list]
        composers = ['Schubert' for _ in xml_list]
...
if __name__ == "__main__":
    ...
    import glob
    xml = "/root/v2/muzic/virtuosonet/data/interval/score/beethoven_woo80_score2.musicxml"
    score_midi = "/root/v2/muzic/virtuosonet/data/interval/score/beethoven_woo80_score2.mid"
    perform_lists = glob.glob("/root/v2/muzic/virtuosonet/data/interval/mid" + '/*.mid')
    print("xml: ", xml)
    piecedata = PieceData(xml_path=xml, perform_lists=perform_lists, score_midi_path=score_midi, save=True)