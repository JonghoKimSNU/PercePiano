...
def handle_args(args):
  ...
  if hasattr(net_param, 'meas_note'):
    args.meas_note = net_param.meas_note # True
  else:
    args.meas_note = False

  if args.session_mode == 'train' or args.session_mode == 'active_learning':
    data_stats = load_dat(Path(args.data_path)/"train"/"stat.pkl") #TODO: 이거 어케생겼나 보자
  else:
...
def batch_to_device(batch, device):
    ...
    if len(batch) == 4:
       batch_x, note_locations, token_type_ids, targets = batch
    elif len(batch) == 6:
        batch_x, batch_y, note_locations, align_matched, pedal_status, edges = batch
    elif len(batch) == 8: # this is used
        batch_x, batch_y, beat_y, meas_y, note_locations, align_matched, pedal_status, edges = batch
    elif len(batch) == 9: # this is used
        batch_x, batch_y, beat_y, meas_y, note_locations, align_matched, pedal_status, edges, targets = batch
    elif len(batch) == 10:
        batch_x, batch_y, beat_y, meas_y, note_locations, align_matched, pedal_status, edges, targets, label_std = batch
    else:
    ...
    if len(batch) == 4:
        return batch_x, note_locations, token_type_ids.to(device).long(), targets.to(device).float()
    elif len(batch) == 6:
        ...
        return batch_x, batch_y, beat_y.to(device), meas_y.to(device), note_locations, align_matched, pedal_status, edges, targets.to(device).float()
    elif len(batch) == 10:
        return batch_x, batch_y, beat_y.to(device), meas_y.to(device), note_locations, align_matched, pedal_status, edges, targets.to(device).float(), label_std
