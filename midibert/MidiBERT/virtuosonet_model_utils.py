...
def span_beat_to_note_num(beat_out, beat_number):
  ...
  # FIXME: it is temporary solution to prevent error
  # Create a mask for valid indices and Apply the mask
  valid_mask = (batch_indices < beat_number.shape[0]) & (note_indices < beat_number.shape[1]) & (beat_indices < beat_out.shape[1])
  batch_indices = batch_indices[valid_mask]
  note_indices = note_indices[valid_mask]
  beat_indices = beat_indices[valid_mask]
...
def run_hierarchy_lstm_with_pack(sequence, lstm, note_numbers = None):
  '''
  ...
  # if batch_note_length.max() < sequence.shape[1]:
  #   batch_note_length = note_numbers.max(dim=-1)[0]
