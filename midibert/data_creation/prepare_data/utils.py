...
def read_items(file_path, return_note_in_seconds=False):
    ...
    for i in range(num_of_instr):
        ...
        for note in notes:
            note_items.append(Item(
                ...
                pitch=note.pitch,
                Type=i))
            note_in_seconds.append(Item(
                name='NoteSeconds',
                start=round(mapping[note.start],4), 
                end=round(mapping[note.end],4),
                velocity=note.velocity,
                pitch=note.pitch,
                Type=i)
            )
    # Zip the two lists together
    zipped_lists = zip(note_items, note_in_seconds)
    # Sort based on the 'start' attribute of note_items
    sorted_lists = sorted(zipped_lists, key=lambda x: x[0].start)
    # Unzip the sorted lists
    note_items, note_in_seconds = zip(*sorted_lists)
    note_items = list(note_items)
    note_in_seconds = list(note_in_seconds)
    ...
    for tick in wanted_ticks:
        if tick in existing_ticks:
            ...
            output.append(Item(
                ...
                Type=-1))
    tempo_items = output
    if return_note_in_seconds:
        return note_items, tempo_items, note_in_seconds
    return note_items, tempo_items, None #what is tempo_items for?
